...
function buildKnowledgeBody(): string {
  const parts: string[] = [];

  parts.push(`【等級制度】（詳細: /hr/grade）\n${sectionsToText(HR_GRADE_SECTIONS)}`);
  ...
  const jitsuNames = HR_EVALUATION_SECTIONS.filter((s) =>
    ...
    .join("／");
  parts.push(
    `【評価制度】（詳細: /hr/evaluation）\n${sectionsToText(evalMain)}\n\n◆ 7つの実 — 面談で見るポイント（各7つ）\n${jitsuNames}\n（各7つの見るポイントの全文は /hr/evaluation に掲載）`
  );
  ...
  parts.push(
    `【ステージ移行】（詳細: /hr/promotion）\n${HR_PROMOTION_INTRO.map(clean).join("\n")}\n\n${transitions}\n\n◆ ${HR_PROMOTION_NOTES_TITLE}\n${HR_PROMOTION_NOTES.map((n) => `- ${clean(n)}`).join("\n")}\n（各移行の「到達状態（各職種10項目）」「3軸の詳細」「対話の問い」の全文は /hr/promotion に掲載）`
  );
...
export function buildHrChatKnowledge(): string {
  return `

【人事制度に関する質問への対応（重要）】
以下の「人事制度の知識」は、mk-training内の人事制度ポータル（ナビ「理念・想い」→ 🧭 人事制度ポータル）の掲載内容と一致しています。人事制度（等級・評価・給与・ステージ移行）に関する質問には、この知識の範囲で正確に答え、回答の末尾に該当ページを案内してください。

回答のルール:
- 「要点を正確に答える＋該当ポータルページへ誘導」を基本形とする。要約で数値・要件を変えないこと。この知識にない数値・要件・条件を生成しないこと。
- ページ案内は次のパスを使う: ポータルトップ /hr ／ 等級制度 /hr/grade ／ 評価制度 /hr/evaluation ／ ステージ移行 /hr/promotion ／ 給与テーブル /hr/salary ／ よくある質問 /hr/faq
- この知識では「到達状態（各職種10項目）の詳細」「7つの実の面談で見るポイント（各7つ）」「対話の問い」を省略している。これらの詳細を聞かれたら、知識にある範囲の要点を答えたうえで /hr/promotion・/hr/evaluation を案内する（ポータルには掲載されているため「記載がない」とは言わない）。
...
2) 賞与の算定方法、または「前段階への戻り」の運用詳細（逓減・調整手当・期間など）への質問:
