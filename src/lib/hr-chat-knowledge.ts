...
// ─── 定型文（指示書129 第4章を140で院長決定により改訂。一言一句このまま使用。改変禁止） ───
// 140の方針: 文書に記載があることはAIが答え切る。「院長へ直接」の個別誘導はしない。
// 個人固有・機密の事項のみ「半期面談（夏）・年次対話（冬）」の決まった接点へ誘導する。
...
// 140変更1: 賃金規程第17条の要点（規程原文から導出・創作しない）＋新制度との関係＋面談接点で締める
export const HR_CHAT_TEMPLATE_BONUS =
  "賞与は、賃金規程（第17条）で次のように定められています。\n【正職員】原則として年1回、6月に支給されます。評価対象期間は前年6月1日〜当年5月31日です。支給日に在籍し、評価対象期間に通常勤務していた方が対象です。金額は本人の能力・勤務成績・勤務態度・出勤状況と医院業績を考慮して決定されます。ただし、業績の著しい低下等により、支給の延期または不支給の場合があります。\n【準職員】原則として賞与は支給されません。ただし、医院業績や貢献度を勘案し、特別に支給される場合があります。\n新しい人事制度（等級・評価ランク）と賞与の関係は、現時点で明文の定めがありません。個別の見込み額やご自身の評価との関係は、半期面談（夏）・年次対話（冬）の機会に確認できます。";

// 140: 「前段階への戻り」運用詳細は賞与と分離（129の4-2から院長個別誘導を除去）
export const HR_CHAT_TEMPLATE_RETURN_DETAIL =
  "『前段階への戻り』の運用詳細（逓減・調整手当・期間など）は、現在整備中です（専門家の確認を経て確定します）。確定後、人事制度ポータルでご案内します。";

export const HR_CHAT_TEMPLATE_PERSONAL =
  "個人の評価や給与額の見込みは、チャットではお答えできません。評価は年次対話での合意で決まり、給与はその積み重ねです。制度の仕組みは人事制度ポータルの各ページで確認できます。個別のご相談は、半期面談（夏）・年次対話（冬）の機会にどうぞ。";

export const HR_CHAT_TEMPLATE_UNKNOWN =
  "その点は、人事制度ポータルなど確認済みの文書に現時点で記載・定めがないため、正確にお答えできません。決まりましたら、人事制度ポータルでご案内します。";
...
回答のルール:
- 優先順位（最重要）: 人事制度（等級・評価・ステージ移行・給与・賞与・昇給）に関する質問では、この知識ブロックと下記の定型文を唯一の正とする。他の知識文書・FAQ・参考情報（就業規則・賃金規程の要約、旧人材育成資料、追加知識など）とこの知識が矛盾する場合は、必ずこの知識を優先し、矛盾する内容を出力しない。
- 誘導の方針（140・院長決定）: 文書に記載があることは、この知識の範囲でAIが答え切る。「院長へ直接ご相談ください」のような院長個別相談への誘導はしない。個人固有・機密の事項（自分の評価・自分の支給額・自分の等級の見込みなど）に限り、「半期面談（夏）・年次対話（冬）の機会に」という決まった接点を案内する。
- 出典・情報源を示す場合は実際の根拠を記載する: ポータル由来は「人事制度ポータル」、賞与など賃金規程由来を含む回答は「賃金規程・人事制度ポータル」を併記する。それ以外の文書名を人事制度の出典として挙げない。
...
2) 賞与・ボーナスに関する質問（「賞与はどう決まる？」「賞与はいくら？」「ボーナスはある？」など、決まり方・算定・金額・支給条件・評価との関係を含む賞与の質問すべて）:
「${HR_CHAT_TEMPLATE_BONUS}」
※この定型文が賃金規程第17条の要点そのものである。これ以外の条文詳細を創作・追加しない。出典は「賃金規程・人事制度ポータル」を併記する。
3) 「前段階への戻り」の運用詳細（逓減・調整手当・期間など）への質問:
「${HR_CHAT_TEMPLATE_RETURN_DETAIL}」
...
4) 個人の評価・給与額の算定・予想への質問:
「${HR_CHAT_TEMPLATE_PERSONAL}」
5) 人事制度について、この知識にもポータルにも記載のないことを聞かれた場合:
